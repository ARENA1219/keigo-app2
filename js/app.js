    ...
    function renderTestQuestion() {
        ...
        mainContent.innerHTML = `
            <div class="test-container">
                <div style="font-size: 0.9rem; color: #60a5fa; font-weight: bold; margin-bottom: 0.5rem;">${sectionTitle}</div>
                <div class="test-progress">問題 ${currentQuestionIndex + 1} / ${currentTestList.length}</div>
                <div style="color: #f8fafc; margin-bottom: 0.5rem; font-weight: 600; opacity: 0.85;">【シーン: ${q.situation}】</div>
                <div style="color: #fbbf24; font-size: 0.95rem; margin-bottom: 2rem; background: rgba(251,191,36,0.1); padding: 0.5rem 0.75rem; border-radius: 6px; display: inline-block;">💡ヒント：${q.hint}</div>
    ...
    function renderTestResult() {
        ...
        let html = `
            <div class="test-container" style="max-width: 800px;">
                ...
                <div class="result-score">${finalScore} <span style="font-size: 1.5rem; color: #cbd5e1;">点</span></div>
                <div class="result-comment ${commentClass}">${comment}</div>
                <div style="color: #cbd5e1; margin-bottom: 2rem;">
        ...
        if (incorrectResults.length > 0) {
            html += `
                <div style="text-align: left; border-top: 1px solid #475569; padding-top: 2rem;">
                    <h3 style="margin-bottom: 1.5rem; color: #f87171;">⚠️ 間違えた問題の復習</h3>
            `;
            
            incorrectResults.forEach(r => {
                html += `
                    <div style="background-color: rgba(248,113,113,0.08); border-left: 4px solid #f87171; padding: 1.5rem; margin-bottom: 1rem; border-radius: 0 8px 8px 0;">
                        <div style="font-weight: bold; margin-bottom: 0.5rem; color: #f8fafc;">Q${r.index}. 【${r.question.situation}】</div>
                        <div style="margin-bottom: 1rem; font-size: 1.1rem; color: #f8fafc;">
                            ${r.question.textPre}<span style="text-decoration: underline; padding: 0 0.5rem; color: #f87171; font-weight: bold;">${r.userAnswer || '(未入力)'}</span>${r.question.textPost}
                        </div>
                        <div style="display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: 1rem;">
                            <div style="background-color: rgba(52, 211, 153, 0.15); color: #34d399; padding: 0.5rem 1rem; border-radius: 4px; font-weight: bold; border: 1px solid rgba(52,211,153,0.3);">
                        ...
                        <div style="color: #cbd5e1; font-size: 0.95rem; line-height: 1.5;">
                            <strong style="color: #f8fafc;">💡解説：</strong> ${r.question.explanation}
                        </div>
                    </div>
                `;
            });
            ...
            html += `</div>`;
             ...
             html += `
                <div style="text-align: left; border-top: 1px solid #475569; padding-top: 2rem; text-align: center;">
                    <h3 style="color: #34d399;">全問正解です！おめでとうございます🎉</h3>
                </div>
            `;
        }
